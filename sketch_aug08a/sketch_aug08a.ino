...
const char* ssid = "asdf8768"; //사용하는 Wifi 이름
const char* password = "asdf1234"; // 비밀번호
const char* mqtt_server = "192.168.43.123"; //mqtt 서버 주소(라즈베리파이에서 ifconfig로 inet 주소 확인)**꼭 수정!!!!!!!!!!!!!
...
void loop() {
  ...
  {
//    float t = bme.readTemperature(); //섭씨 온도 가져오기    float h = bme.readHumidity(); //습도 가져오기
    ...
    sprintf(message, "{\"pm1\":%d,\"pm25\":%d,\"pm10\":%d}", pm1, pm25, pm10); //JSON 문자열의 형식으로 온도, 습도, 미세먼지 데이터 담기
